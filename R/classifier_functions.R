...
confusion_matrix_handmade <- function(temp_exposures_df, confusion_df, LCD_result_df, match_value){
  ...
  confusion_vector <- c(confusion_value = confusion_value, all_sample = dim(LCD_result_df)[2],
                        matches = match_value, miss_FL_DLBCL = temp_FL_DLBCL, miss_BL_non =  temp_BL_non_BL)
...
calculate_FI <- function(LCD_result_df){
  ...
  if(length(unique(conf_data)) == number_of_signatures){
    ...
    FI_vector <- 0
    FI <- 0
    warning("ConfusionMatrix could not been used: levels of reference more than levels of matches")
  }
...
calculate_for_classifier_range <- function(signature_matrix, 
  ...
  all_results_list <- lapply(test_odds, FUN=function(thres.outer){
    results_list <- lapply(test_dist, FUN=function(thres.inner){
      all.signature <- which(k.dist > thres.inner[1] & k.dist < k.dist_max & oddsVar < thres.outer[1] & oddsVar > oddsVar_min) 
      
      if(length(all.signature) >= 2){
        #####SAMPLE MATRIX
        sample_matrix <- matrix_V[all.signature,]
        sample_matrix <- data.frame(apply(sample_matrix, 2, function(x) x/sum(x)))
        
        #####SIGNATURE MATRIX
        signature_matrix <- signature_matrix[all.signature,]
        signature_matrix <- data.frame(apply(signature_matrix, 2, function(x) x/sum(x)))
      ...
      results <- list(Counter = (counter/match_value), Match = (match_value/sample_size),
                      Number_of_genes = (length(all.signature)), F1_for_each_class = FI_vector,
                      F1_mean = FI)
      return(results)
    })
...
extract_signature_specific_genes <- function(genes_df, annotation_df, match_by = "external_gene_name"){
  if(dim(genes_df)[1] == dim(annotation_df)[1]){
    ...
    match_int <- match(tolower(annotation_df[,col_int]), tolower(rownames(genes_df)))
