  ...
  for(current_rank in ranks[-1]){
    ...
    if(rank_flag) {
      ...
      if(any(start_coords < 0)){
        node_ypos[my_nodes] <-
          rank(unlist(yCoords), ties.method = "first") - 0.5 * (current_rank + 1)
      }
    } else {
...
generateRiverplot <- function(nmf.exp, edges.cutoff = 0, useH=FALSE,
  ...
  edges <- do.call(rbind, lapply(rank.min:(rank.max-1), function(i){
    df <- data.frame(N1=rep(sapply(1:i, function(n) paste(i,"_S", n, sep="")), each=i+1),
                     N2=rep(sapply(1:(i+1), function(n) paste(i+1,"_S", n, sep="")), time=i),
